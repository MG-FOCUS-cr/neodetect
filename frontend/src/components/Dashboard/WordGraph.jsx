...
const WordGraph = () => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-8 flex flex-col h-full min-h-[300px] md:min-h-[400px]">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-8 gap-2 sm:gap-0">
                <div>
                    <h2 className="text-lg md:text-xl font-bold text-slate-900 flex items-center gap-2">
                        <BarChart2 className="text-brand-600" size={20} />
                        Tendances Lexicales
                    </h2>
                    <p className="text-slate-500 text-xs md:text-sm mt-1">Termes les plus analysés cette semaine</p>
                </div>
                <div className="bg-green-50 text-green-700 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-bold flex items-center gap-1">
                    <TrendingUp size={12} /> +24% trafic
                </div>
            </div>

            <div className="flex-1 flex items-end justify-between gap-2 md:gap-4 h-48 md:h-64 border-b border-slate-100 pb-4 relative">
                ...
                {data.map((item, index) => (
                    <div key={index} className="flex flex-col items-center justify-end w-full h-full group z-10 relative">
                        ...
                        <motion.div
                            ...
                            className={`w-full max-w-[40px] md:max-w-[60px] rounded-t-lg md:rounded-t-xl ${item.color} opacity-80 group-hover:opacity-100 transition-all relative overflow-hidden`}
                        >
                            <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-t from-black/10 to-transparent"></div>
                        </motion.div>

                        {/* Label */}
                        <span className="mt-2 md:mt-4 text-[10px] md:text-xs font-medium text-slate-500 uppercase tracking-wide truncate w-full text-center">
                            {item.label}
                        </span>
                    </div>
                ))}
